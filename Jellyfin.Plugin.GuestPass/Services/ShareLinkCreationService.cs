...
/// <summary>Creates durable GuestPass records and applies the temporary tag.</summary>
public sealed class ShareLinkCreationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var record = new ShareLinkRecord
        {
            ...
            OneUse = oneUse,
            AllowedTag = $"guestpass-{Guid.NewGuid():N}",
            CleanupAttempts = 0
        };
        ...
        catch (Exception ex)
        {
            ...
            await _store.UpdateAsync(record, cancellationToken).ConfigureAwait(false);
            _logger.LogWarning(ex, "GuestPass: failed to finish creation for record {RecordId}.", record.Id);
            throw;
        }
    }
}
