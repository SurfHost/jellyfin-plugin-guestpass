...
public sealed class ShareTokenService
{
    ...
    {
        _secretPath = Path.Combine(applicationPaths.DataPath, "guestpass", "token-secret.key");
        _logger = logger;
    }
    ...
    private async Task<byte[]> GetSecretAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (File.Exists(_secretPath))
            {
                ...
                {
                    _logger.LogWarning(ex, "GuestPass: could not load the token secret; a new one will be generated.");
                }
            }
